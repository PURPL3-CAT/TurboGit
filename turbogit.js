...
async function exportProject(vm) {
    ...
    for (const target of originals) {
        ...
        const blocksFile = await spriteFolder.getFileHandle("blocks.json", { create: true });
        ...
        // === COSTUMES ===
        //
        const costumesFolder = await spriteFolder.getDirectoryHandle("costumes", { create: true });

        const costumeMeta = [];

        for (const costume of sprite.costumes) {
            const filename = `${costume.name}.${costume.dataFormat}`;
            const fileHandle = await costumesFolder.getFileHandle(filename, { create: true });
        ...
        const costumeMetaFile = await costumesFolder.getFileHandle("costumes.json", { create: true });
        ...
        // === SOUNDS ===
        //
        const soundsFolder = await spriteFolder.getDirectoryHandle("sounds", { create: true });
        ...
        for (const sound of sprite.sounds) {
            ...
            const fileHandle = await soundsFolder.getFileHandle(filename, { create: true });
            const writable = await fileHandle.createWritable();
        ...
        const soundMetaFile = await soundsFolder.getFileHandle("sounds.json", { create: true });
...
async function pullFromFolder(vm) {
    ...
    for (const { name: spriteName, handle: spriteDir } of folderSprites) {
        ...
        const costumeMeta = JSON.parse(await readTextFile(costumesDir, "costumes.json"));
        ...
        //
        const existing = vm.runtime.targets.find(t => t.isOriginal && t.sprite.name === spriteName);
...
function md5(data) {
    ...
    function md51(s) {
        let i, n = s.length, state = [1732584193, -271733879, -1732584194, 271733878];
    ...
    function rhex(n) {
        let s = '', j = 0;
        for (; j < 4; j++) s += '0123456789abcdef'[(n >> (j * 8 + 4)) & 0x0F] + '0123456789abcdef'[(n >> (j * 8)) & 0x0F];
        return s;
    }
